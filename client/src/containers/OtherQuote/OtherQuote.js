...
function OtherQuote(){
    ...
    useEffect(() => {
        setProfileData(data)
        if(data){
            localStorage.setItem("otherQuote", JSON.stringify(data))
        } else {
            let collection = localStorage.getItem('otherQuote');
        ...
        <div className="bg-gray-200 dark:bg-slate-900 font-sans text-gray-700">
            <div className="container md:mx-auto mx-auto p-8 flex" style={{minHeight:'100vh'}}>
                <div className="bg-white mt-20 dark:bg-slate-700 rounded-lg overflow-hidden min-w-full shadow-2xl">
                    ...
                    <h1 className="text-3xl text-center mt-7 font-black dark:text-slate-200">{profileData?.users.firstName} {profileData?.users.lastName}</h1>
